...
async function run() {
  ...
  const rowsLines = await page.evaluate(() => {
    ...
    for (const row of rows) {
      ...
      results.push({
        rawItineraryText: cells[itineraryIdx] || ''
      });
    }
    return results;
  });

  console.log('--- ITINERARY NEWLINE DUMP ---');
  for (let i = 0; i < Math.min(rowsLines.length, 10); i++) {
    console.log(`\nDeal ${i}:`);
    const lines = rowsLines[i].rawItineraryText.split('\n');
