...
async function run() {
  ...
  for (let p = 1; p <= 15; p++) {
    ...
    for (const text of pagePromos) {
      ...
      lines.forEach((line, idx) => {
        if (idx > 0) { // Skip the first line which is the itinerary title
          allPromos.add(line);
          if (line.toLowerCase().includes('tpi') || line.toLowerCase().includes('travel planners')) {
            tpiMatches++;
            console.log(`[FOUND TPI] Page ${p}: "${line}" (Full cell text: "${text.replace(/\s+/g, ' ')}")`);
          }
        }
      });
    }
  ...
  console.log(`\nScan finished. Total unique promo lines collected: ${allPromos.size}`);
  console.log('Unique promo lines list:');
  console.log(Array.from(allPromos));
  console.log(`Total TPI matches: ${tpiMatches}`);
